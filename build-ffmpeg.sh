...
cd builder
chmod -R 0777 . && chmod +x ./build-android-2.3.sh && chmod +x ./build-android-4.0.4.sh
...
if [[ -z $1 ]]; then
    ...
    echo;
    echo "[ERROR] Wrong argument: ./build-ffmpeg.sh [version == '2.3' or '4.0.4']"
    exit 1;
fi;
...
if [[ -z $1 ]]; then
    cp ./ffmpeg-2.3/android/arm64-v8a/libffmpeg.so ../../app/src/main/jniLibs/arm64-v8a
