#!/bin/bash
#  FFmpeg 0.6.5 & 4.0.4 for Android automatic build script using builder by Tinelix
...
cd builder
chmod -R 0777 . && chmod +x ./build-android-2.6.sh && chmod +x ./build-android-0.6.5.sh && chmod +x ./build-android-0.8.5.sh
...
if [[ -z $1 ]]; then
    ...
    ./build-android-0.6.5.sh armv6 r7
    ./build-android-0.6.5.sh armv7 r7
elif [ $1 == "0.8.5" ]; then
    ...
    echo;
    echo "[ERROR] Wrong argument: ./build-ffmpeg.sh [version == '2.6' or '0.6.5' or '0.8.5']"
    exit 1;
fi;
...
if [[ -z $1 ]]; then
    ...
    cp ./ffmpeg-0.6.5/android/armeabi/libffmpeg.so ../../../app/src/main/jniLibs/armeabi
    cp ./ffmpeg-0.6.5/android/armeabi-v7a/libffmpeg.so ../../../app/src/main/jniLibs/armeabi-v7a
elif [ $1 == "0.8.5" ]; then
    cp ./ffmpeg-0.8.5/android/armeabi/libffmpeg.so ../../../app/src/main/jniLibs/armeabi
    cp ./ffmpeg-0.8.5/android/armeabi-v7a/libffmpeg.so ../../../app/src/main/jniLibs/armeabi-v7a
else
